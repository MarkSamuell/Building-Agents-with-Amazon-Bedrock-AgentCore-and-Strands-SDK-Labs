...
**Lab 2's agent, deployed and torn down 2026-08-29.** `direct_code_deploy` of `lab_2/starter.py` as `WanderBot3` produced runtime `arn:aws:bedrock-agentcore:us-east-1:484086766087:runtime/WanderBot3-jJXNSUD2VQ` (version 1, `READY`), IAM role `AmazonBedrockAgentCoreSDKRuntime-us-east-1-e0238e14b6`, `WanderBot3/deployment.zip` (45.7 MiB) in bucket `bedrock-agentcore-codebuild-sources-484086766087-us-east-1`, and log group `/aws/bedrock-agentcore/runtimes/WanderBot3-jJXNSUD2VQ-DEFAULT`. No ECR repo and no CodeBuild project -- direct code deploy creates neither. **All removed at 12:02, verified empty across runtimes, IAM, S3 and log groups.** This teardown raised no `destroy` warnings, because with no CodeBuild role there was nothing for it to skip; only the bucket and log group needed deleting by hand.

**Two things lab 2 settled.** Non-Python files **do** ship with a direct-code zip -- the deployed agent read real records out of `lab_2/datasets/*.json`, so the package includes everything under the source directory. And `bedrock-agentcore-starter-toolkit` must **not** go in `requirements.txt`: it is the `agentcore` CLI, a dev tool, and listing it shipped ~9 MB of CLI into the runtime. Lab 1's package was 54.76 MB against lab 2's 45.68 MB for the same four imports. `lab_1/requirements.txt` has been corrected.
